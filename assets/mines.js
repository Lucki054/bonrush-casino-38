...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-af2c0a3b";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-af2c0a3b">'+'<span class="tile-face-af2c0a3b tile-front-af2c0a3b"></span>'+'<span class="tile-face-af2c0a3b tile-back-af2c0a3b"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-af2c0a3b")){return;}
var back=tile.querySelector(".tile-back-af2c0a3b");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-af2c0a3b");if(isMine){tile.classList.add("boom-af2c0a3b");}
if(dim){tile.classList.add("dimmed-af2c0a3b");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-af2c0a3b")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-af2c0a3b");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-af2c0a3b")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-af2c0a3b");void panelEl.offsetWidth;panelEl.classList.add("jolt-af2c0a3b");}
endRound();}
